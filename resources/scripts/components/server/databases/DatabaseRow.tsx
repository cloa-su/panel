...
export default ({ database, className }: Props) => {
    ...
    const phpMyAdmin = () => {
        window.location.href = 'https://phpmyadmin.cloa.su';
    };
	...
	return (
        <>
            ...
            >
                {
                    ({ isSubmitting, isValid, resetForm }) => (
                        ...
                        >
                            ...
                            <p css={tw`text-sm`}>
                            Удаление базы данных — это постоянное действие, его нельзя отменить. Это навсегда удалит базу данных <strong>{database.name}</strong> и все связанные с ней данные.
                            </p>
            ...
            <Modal visible={connectionVisible} onDismissed={() => setConnectionVisible(false)}>
                ...
                <h3 css={tw`mb-6 text-2xl`}>Подробности подключения к базе данных</h3>
                <div>
            ...
            <GreyRowBox $hoverable={false} className={className} css={tw`mb-2`}>
                ...
                <div css={tw`ml-8 text-center hidden md:block`}>
                    ...
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>Конечная точка</p>
                </div>
                <div css={tw`ml-8 text-center hidden md:block`}>
                    <p css={tw`text-sm`}>{database.allowConnectionsFrom}</p>
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>Соединения из</p>
                </div>
                <div css={tw`ml-8 text-center hidden md:block`}>
                    <CopyOnClick text={database.username}><p css={tw`text-sm`}>{database.username}</p></CopyOnClick>
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>Имя пользователя</p>
                </div>
                <div css={tw`ml-8`}>
